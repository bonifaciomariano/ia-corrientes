// ═══════════════════════════════════════════════════════
// DATA.JS · Estrategias de IA 2026
// Datos base — pueden sobreescribirse desde el Gist
// ═══════════════════════════════════════════════════════
...
const FASES = [
  {
    ...
    ejes:[
      "Institucionalización: conformación y puesta en marcha de la Mesa de Trabajo sobre IA con designación de enlaces de las áreas clave.",
      "Relevamiento: diagnóstico integral para identificar infraestructura disponible, normativa vigente y procesos cuello de botella.",
      "Validación y Consenso: consultas con expertos externos y taller de sensibilización para definir expectativas y alinear visión política con estándares técnicos."
    ],
  ...
  },
  {
    ...
    ejes:[
      "Normativa: confección del Protocolo de uso ético con principios de transparencia, privacidad y supervisión humana obligatoria.",
      "Estrategia y Vinculación: formalización del Libro Blanco y rondas de vinculación con universidades y sector privado.",
      "Capacitación: despliegue del plan de formación para nivelar el conocimiento de toda la planta."
    ],
  ...
  },
  {
    ...
    ejes:[
      "Prototipado e Implementación: selección estratégica y desarrollo funcional de herramientas.",
      "Estandarización Operativa: creación de guías prácticas y biblioteca de prompts institucional.",
      "Formación Avanzada: trayectos formativos segmentados por roles."
    ],
  ...
  },
  {
    ...
    ejes:[
      "Informe de Gestión: balance anual detallado que permite evaluar el cumplimiento de hitos.",
      "Planificación 2027: diseño de la hoja de ruta y esquema de gastos para el próximo ciclo."
    ],
    t_start:6,t_end:6,tipo:"meses"
  }
];

// Productos F1 — estructura base (sobreescribible desde Gist)
const PRODUCTOS_F1_BASE = [
  {
    id:"f1p1",num:"01",nombre:"Asesoramiento Experto",
    desc:"Consultas técnicas cerradas con especialistas para validar el diagnóstico inicial y alimentar la visión estratégica de las autoridades.",
    hito:{nombre:"Informe de visión estratégica",fecha:"Semana del 6 de abril"},
    t_start:1,t_end:3,
    subproductos:[
      {id:"f1p1s1",nombre:"Reuniones con especialistas en modernización parlamentaria",t_start:1,t_end:3,es_hito:false},
  ...
  },
  {
    ...
    t_start:2,t_end:11,
    subproductos:[
      {id:"f1p3s1",nombre:"Diseño: integrantes, dinámica, frecuencia, agenda y objetivos",t_start:2,t_end:4,es_hito:false},
      {id:"f1p3h1",nombre:"1ra reunión de la Mesa",fecha:"Semana del 20 de abril",t_start:5,t_end:5,es_hito:true},
      {id:"f1p3s2",nombre:"Reuniones quincenales de seguimiento y validación",t_start:5,t_end:11,es_hito:false}
    ]
  },
  {
    ...
    t_start:1,t_end:8,
    subproductos:[
      {id:"f1p4s1",nombre:"1.1 Diseño del instrumento: mapeo de actores, matriz de evaluación e indicadores",t_start:1,t_end:2,es_hito:false},
      {id:"f1p4s2",nombre:"1.2 Relevamiento: entrevistas, flujograma y análisis de calidad de datos",t_start:3,t_end:5,es_hito:false},
      {id:"f1p4s3",nombre:"1.3 Análisis: hallazgos, detección de brechas y recomendaciones",t_start:6,t_end:8,es_hito:false},
  ...
  },
  {
    ...
    desc:"Herramienta clave para la toma de decisiones, orientada a conocer el estado de situación, identificar oportunidades y anticipar desafíos internos.",
...
const PRODUCTOS_F2_BASE = [
  {id:"f2p1",nombre:"Rondas de vinculación (universidades, OSC, sector privado, organismos internacionales)",t_start:0,t_end:2,t_hito:2,hito:{nombre:"Minutas de acuerdos",fecha:"Ago 2026"}},
  {id:"f2p2",nombre:"Protocolo de ética y uso responsable de IA",t_start:0,t_end:2,t_hito:2,hito:{nombre:"Protocolo aprobado",fecha:"Ago 2026"}},
  ...
  {id:"f2p4",nombre:"Formación general del personal",t_start:0,t_end:2,t_hito:null,hito:null}
];

const PRODUCTOS_F3_BASE = [
  {id:"f3p1",nombre:"Selección estratégica de casos de uso",t_start:3,t_end:3,t_hito:null,hito:null},
  {id:"f3p2",nombre:"Prototipado y desarrollo funcional de pilotos",t_start:3,t_end:5,t_hito:5,hito:{nombre:"Prototipo funcional",fecha:"Nov 2026"}},
  {id:"f3p3",nombre:"Manual operativo y biblioteca de prompts",t_start:3,t_end:5,t_hito:null,hito:null},
  {id:"f3p4",nombre:"Formación avanzada por roles",t_start:3,t_end:5,t_hito:null,hito:null}
];

const PRODUCTOS_F4_BASE = [
  {id:"f4p1",nombre:"Informe de gestión anual con métricas de impacto",t_start:6,t_end:6,t_hito:6,hito:{nombre:"Informe de gestión",fecha:"Dic 2026"}},
...
// Estado inicial — sobreescrito por Gist o localStorage
const ESTADO_INICIAL = {
  version:2,
  tareas:{},
  actividad:[],
  descripciones:{},
  // cronograma dinámico — null = usar base
  cronograma: null,
  config:{gistId:"",gistToken:"",editorPassword:"ia2026"}
};

// Runtime: productos activos (base o editados)
let PRODUCTOS_F1 = JSON.parse(JSON.stringify(PRODUCTOS_F1_BASE));
let PRODUCTOS_F2 = JSON.parse(JSON.stringify(PRODUCTOS_F2_BASE));
let PRODUCTOS_F3 = JSON.parse(JSON.stringify(PRODUCTOS_F3_BASE));
let PRODUCTOS_F4 = JSON.parse(JSON.stringify(PRODUCTOS_F4_BASE));
let TODOS_LOS_HITOS = JSON.parse(JSON.stringify(TODOS_LOS_HITOS_BASE));

// Reconstruir TODAS_LAS_TASKS dinámicamente
function buildAllTasks() {
  const tasks = [];
  PRODUCTOS_F1.forEach(p => {
    tasks.push({id:p.id,faseId:"f1",nombre:p.nombre,tipo:"producto"});
    p.subproductos.forEach(s => tasks.push({id:s.id,faseId:"f1",nombre:s.nombre,tipo:s.es_hito?"hito":"sub"}));
  });
  [...PRODUCTOS_F2,...PRODUCTOS_F3,...PRODUCTOS_F4].forEach(p => {
    const fid = p.id.startsWith("f2")?"f2":p.id.startsWith("f3")?"f3":"f4";
    tasks.push({id:p.id,faseId:fid,nombre:p.nombre,tipo:"producto"});
  });
  return tasks;
}
let TODAS_LAS_TASKS = buildAllTasks();

function applyDynamicCronograma(cronData) {
  if (!cronData) return;
  if (cronData.f1) PRODUCTOS_F1 = JSON.parse(JSON.stringify(cronData.f1));
  if (cronData.f2) PRODUCTOS_F2 = JSON.parse(JSON.stringify(cronData.f2));
  if (cronData.f3) PRODUCTOS_F3 = JSON.parse(JSON.stringify(cronData.f3));
  if (cronData.f4) PRODUCTOS_F4 = JSON.parse(JSON.stringify(cronData.f4));
  TODAS_LAS_TASKS = buildAllTasks();
}

function saveCronogramaToState(STATE) {
  STATE.cronograma = {
    f1: JSON.parse(JSON.stringify(PRODUCTOS_F1)),
    f2: JSON.parse(JSON.stringify(PRODUCTOS_F2)),
    f3: JSON.parse(JSON.stringify(PRODUCTOS_F3)),
    f4: JSON.parse(JSON.stringify(PRODUCTOS_F4))
  };
}

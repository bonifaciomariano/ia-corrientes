...
function ganttFaseRow(fase,NW,NM){
  return `<tr style="background:#1A2E1A;">
    <td style="padding:9px 14px;font-size:10px;font-weight:700;color:#C5E830;letter-spacing:.1em;text-transform:uppercase;border-right:1px solid rgba(255,255,255,.08);">${fase.num} · ${fase.nombre} · ${fase.periodo}</td>
...
function ganttSubRow(sub,NW,NM){
  ...
  const editAttr = IS_EDITOR&&!sub.es_hito ? `ondblclick="startInlineEdit(event,'${sub.id}','sub')"` : "";
  return `<tr style="background:${rowBg};border-bottom:1px solid #E0E0E0;cursor:pointer;"
    ...
    <td style="padding:7px 12px 7px 26px;font-size:11px;color:${sub.es_hito?"#2D5016":"#444"};font-weight:${sub.es_hito?600:300};border-right:1px solid #E0E0E0;" ${editAttr}>
      ...
      ${IS_EDITOR&&!sub.es_hito?`<span class="inline-hint" title="Doble click para editar">✎</span>`:""}
...
function openDetail(taskId){
  ...
  let nombre=taskId, fase="", meta="", currentDesc="", ts=null, te=null, isF1Sub=false, prodId=null, isSub=false, isLater=false;
  ...
  PRODUCTOS_F1.forEach(p=>{
    ...
    p.subproductos.forEach(s=>{
      if(s.id===taskId){ nombre=s.nombre; fase=`Fase 1 · ${p.nombre}`; meta=s.fecha||""; currentDesc=getDesc(s.id,""); ts=s.t_start; te=s.t_end; prodId=p.id; isSub=true; isF1Sub=true; }
    });
  });
  ...
  if(rangeEl && ts!==null && te!==null){
    ...
    const opts = isWeek
      ? SEMANAS_F1.map((w,i)=>`<option value="${i}" ${i===ts?"selected":""}>${w}</option>`).join("")
      : MESES_POSTERIORES.map((m,i)=>`<option value="${i}" ${i===ts?"selected":""}>${m.label}</option>`).join("");
    const optsEnd = isWeek
      ? SEMANAS_F1.map((w,i)=>`<option value="${i}" ${i===te?"selected":""}>${w}</option>`).join("")
      : MESES_POSTERIORES.map((m,i)=>`<option value="${i}" ${i===te?"selected":""}>${m.label}</option>`).join("");

    rangeEl.innerHTML = IS_EDITOR ? `
      <div class="detail-label">Período</div>
      <div class="range-row">
        <div class="range-field"><label>Inicio</label><select id="rangeStart" class="range-select">${opts}</select></div>
        <div class="range-arrow">→</div>
        <div class="range-field"><label>Fin</label><select id="rangeEnd" class="range-select">${optsEnd}</select></div>

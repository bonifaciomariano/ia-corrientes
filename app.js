...
function addActividad(texto, tipo="estado"){ if(tipo==="edicion") return; STATE.actividad.unshift({texto,tipo,ts:new Date().toISOString()}); if(STATE.actividad.length>50)STATE.actividad=STATE.actividad.slice(0,50); }
...
function renderDashboard() {
  ...
  const hitosAll = TODOS_LOS_HITOS.filter(h=>{
    if(isF && h.fase!==`Fase ${DASHBOARD_FASE_FILTER+1}`) return false;
    return true;
  });
  const hitosPending = hitosAll.filter(h=>getTaskState(h.taskId).status!=="completado");
  const hitosDone    = hitosAll.filter(h=>getTaskState(h.taskId).status==="completado");
  const actFiltrada  = STATE.actividad.filter(a=>a.tipo==="estado"||a.tipo==="nota"||!a.tipo);

  document.getElementById("tab-dashboard").innerHTML = `
  <div class="dash4-grid">

    <div class="dash-card dash-card--hero dash4-q1">
    ...
    <div class="dash4-q2">
      <div class="dash-phases-grid">
        ${fids.map((fid,i)=>{
          const fp=phasePct(fid),tasks=TODAS_LAS_TASKS.filter(t=>t.faseId===fid),done=tasks.filter(t=>getTaskState(t.id).status==="completado").length,sel=i===DASHBOARD_FASE_FILTER;
          return `<div class="dash-card phase-card ${sel?"phase-card--selected":""}" onclick="setDashFilter(${sel?"null":i})" style="cursor:pointer;">
    ...
    <div class="dash-card dash4-q3">
      <div class="card-title">Hitos pendientes${isF?` · Fase ${DASHBOARD_FASE_FILTER+1}`:""}</div>
      ${hitosPending.length?hitosPending.map(h=>`
        ...
        </div>`).join(""):`<p class="empty-state">Sin hitos pendientes 🎉</p>`}
      ${hitosDone.length?`
        <div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--line);">
          <div style="font-size:10px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:var(--grey-light);margin-bottom:8px;">Completados</div>
          ${hitosDone.map(h=>`
            <div class="hito-item hito-item--done" onclick="openDetail('${h.taskId}')">
              <div class="hito-diamond done"></div>
              <div class="hito-info">
                <div class="hito-item-name" style="text-decoration:line-through;opacity:.6;">${h.nombre}</div>
                <div class="hito-item-date">${h.fase}</div>
              </div>
            </div>`).join("")}
        </div>`:""}
    </div>

    <div class="dash-card dash4-q4">
      <div class="card-title">Actividad reciente</div>
      ${actFiltrada.length?actFiltrada.slice(0,8).map(a=>`
        <div class="activity-item">
          <div class="activity-dot" style="background:${a.tipo==="nota"?"#FFC107":"var(--green)"}"></div>
          <div>
...
function ganttFaseRow(fase,NW,NM){
  const editAttr = IS_EDITOR ? `onclick="handleRowClick(event,'${fase.id}','fase')"` : "";
  return `<tr style="background:#1A2E1A;${IS_EDITOR?'cursor:pointer':''}">
    <td style="padding:9px 14px;font-size:10px;font-weight:700;color:#C5E830;letter-spacing:.1em;text-transform:uppercase;border-right:1px solid rgba(255,255,255,.08);" ${editAttr}>
      ...
      ${IS_EDITOR?`<span class="inline-hint" style="color:rgba(255,255,255,.3);">✎ doble click</span>`:""}
    </td>
...
function ganttProdRow(id,nombre,ts,te,st,isF1,NW,NM){
  const rowClick = IS_EDITOR ? `onclick="handleRowClick(event,'${id}','prod')"` : `onclick="openDetail('${id}')"`;
...
function ganttSubRow(sub,NW,NM){
  ...
  const rowClick = IS_EDITOR ? `onclick="handleRowClick(event,'${sub.id}','sub')"` : `onclick="openDetail('${sub.id}')"`;
...
function ganttLaterRow(prod,st,NW,NM){
  const rowClick = IS_EDITOR ? `onclick="handleRowClick(event,'${prod.id}','later')"` : `onclick="openDetail('${prod.id}')"`;
  return `<tr style="background:#fff;border-bottom:1px solid #E0E0E0;cursor:pointer;"
    onmouseover="this.style.background='#EAF0E3'" onmouseout="this.style.background='#fff'"
    ${rowClick}>
    <td style="padding:9px 12px 9px 14px;font-size:12px;font-weight:600;color:#1A1A1A;border-right:1px solid #E0E0E0;border-left:3px solid #2D5016;">
...
// ── Inline edit ───────────────────────────────────────
// Click timer — distinguishes single click (open panel) from double click (inline edit)
const _clickTimers = {};

function handleRowClick(event, id, tipo){
  if(_clickTimers[id]){
    // Second click arrived — this is a double click
    clearTimeout(_clickTimers[id]);
    delete _clickTimers[id];
    if(IS_EDITOR) startInlineEdit(event, id, tipo);
  } else {
    // First click — wait 280ms before deciding it's a single click
    _clickTimers[id] = setTimeout(()=>{
      delete _clickTimers[id];
      openDetail(id);
    }, 280);
  }
...
function updateTaskName(id, newName, tipo){
  ...
  addActividad(`Renombrado: "${newName.slice(0,40)}"`, "edicion");
  saveToGist(false);
}
...
function saveDetail(){
  ...
  // Log note activity if notes were added/changed
  const prevNotas = getTaskState(DETAIL_TASK_ID).notas || "";
  if(notas.trim() && notas.trim() !== prevNotas.trim()){
    const t = TODAS_LAS_TASKS.find(t=>t.id===DETAIL_TASK_ID);
    const nombre = t ? t.nombre.slice(0,40) : DETAIL_TASK_ID;
    addActividad(`Nota en "${nombre}"`, "nota");
  }
